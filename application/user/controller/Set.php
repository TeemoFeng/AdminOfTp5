...
class Set extends Common
{
    ...
    public function bindMobile()
    {
        ...
        //获取手机验证码
        $mobile_send = session('mobile_' . $mobile);
        ...
        if ($mobile_code != $code) {
            ...
            session('mobile_' . $mobile, null); //清除session
        }
    ...
    {
        $data = input('post.');
        if(empty($data['username']) || empty($data['identity'])){
            return ['code' => 0, 'msg' => '姓名和身份证号不能为空'];
        }

        if(!is_idcard($data['identity'])){
            return ['code' => 0, 'msg' => '身份证号不正确'];
        }
        $user_id = session('user.id');
        if (Db::name('users')->where('id', $user_id)->update($data) !== false) {
            $result['msg'] = '认证成功!';
            $result['code'] = 1;
        } else {
            $result['msg'] = '认证失败!';
